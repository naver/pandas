######
# Modified version of: https://github.com/lvis-dataset/lvis-api/blob/master/lvis/eval.py
######
...
class LVISEval:
    ...
    def summarize(self):
        ...
        self.results_per_class = OrderedDict()
        for class_id in range(len(self.params.cat_ids)):
            container = {}
            container["AP"] = self._summarize('ap', subset_class_ids=[class_id])
            container["AP50"] = self._summarize('ap', iou_thr=0.50, subset_class_ids=[class_id])
            self.results_per_class[class_id] = container
